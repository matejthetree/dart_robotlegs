...
class MediatorFactory {
  ...
  MediatorFactory(this._injector, [this._manager = null]) {
    if (_manager == null) _manager = new MediatorManager(this);
  }
  ...
  List<dynamic> createMediators(dynamic item, Type type, List mappings) {
    ...
    mappings.forEach((mapping) {
      ...
      if (mediator != null) createdMediators.add(mediator);
    });
  ...
  void removeMediators(dynamic item) {
    ...
    if (mediators == null || mediators.length == 0) return;

    mediators.forEach((key, mapping) {
      _manager.removeMediator(
          mediators[mapping], item, mapping as IMediatorMapping);
    });
  ...
  dynamic _createMediator(dynamic item, IMediatorMapping mapping) {
    ...
    if (mediator != null) return mediator;

    if (mapping.guards.length == 0 ||
        guardsApprove(mapping.guards, _injector)) {
  ...
  void addMediator(dynamic mediator, dynamic item, IMediatorMapping mapping) {
    if (_mediators[item] == null) _mediators[item] = new Map();

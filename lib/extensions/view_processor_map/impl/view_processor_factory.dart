...
class ViewProcessorFactory implements IViewProcessorFactory {
  ...
  void runProcessors(
    ...
    processorMappings.forEach((mapping) {
  ...
  void runAllUnprocessors() {
    _listenersByView.forEach((key, removalHandlers) {
      final int length = removalHandlers.length;

      for (int i = 0; i < length; i++) removalHandlers[i](null);
    });
  }
  ...
  void _removeHandlerFromView(dynamic view, Function handler) {
    if ((_listenersByView[view] != null) &&
        (_listenersByView[view].length > 0)) {
      _listenersByView[view].remove(handler);
      if (_listenersByView[view].length == 0) _listenersByView.remove(view);
    }
  }
}
